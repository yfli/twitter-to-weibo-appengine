...
from tweibopy.binder import bind_api
from tweibopy.error import TweepError
from tweibopy.parsers import ModelParser
from tweibopy.utils import list_to_csv
...
class API(object):
    ...
    def __init__(self, auth_handler=None,
            host='api.t.sina.com.cn', search_host='search.twitter.com',
    ...
    """ statuses/upload """
    def upload_status (self, status, filename, filecontent):
        headers, post_data = API._pack_image(filename, 5000, filecontent=filecontent, msg=status)
        return bind_api(
            path = '/statuses/upload.json',
            method = 'POST',
            payload_type = 'status',
            allowed_param = ['status', 'pic', 'lat', 'long', 'source' ],
            require_auth = True
    ...
    """ Internal use only """
    @staticmethod
    def _pack_image(filename, max_size, filecontent=None, msg=None):
        ...
        payload_data['pic'] = MultipartParam('pic', filename=filename,
                filetype=file_type, value = filecontent )
        body, headers = multipart_encode(payload_data)
